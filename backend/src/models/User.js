...
class User {
  ...
  static async getLeaderboard(limit = 50, seasonId = null) {
    let query;
    let params = [limit];

    if (seasonId) {
      query = `
        SELECT u.id, u.username, u.display_name, u.avatar_url, uss.points, u.level,
               COUNT(w.id) as workout_count, MAX(w.created_at) as last_workout_date
        FROM users u
        JOIN user_seasonal_stats uss ON u.id = uss.user_id
        LEFT JOIN workouts w ON u.id = w.user_id AND w.created_at BETWEEN (SELECT start_date FROM seasons WHERE id = $2) AND (SELECT end_date FROM seasons WHERE id = $2)
        WHERE uss.season_id = $2
        GROUP BY u.id, uss.points
